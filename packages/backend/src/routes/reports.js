...
// ── Helper totaux d'un mois (partagé par evolution + annual) ──────
async function getMonthTotals(userId, month, year) {
  ...
  return {
    ...
    totalExpenses,
    totalIncomes,
    expenses:          totalExpenses,   // alias pour compatibilité front
    incomes:           totalIncomes,    // alias pour compatibilité front
    ...
    const result = await Promise.all(
      Array.from({ length: 12 }, (_, i) => i + 1).map(async m => {
        ...
        return {
          ...
          expenses:          totalExp,
          incomes:           totalInc,
          totalExpenses:     totalExp,   // alias
          totalIncomes:      totalInc,   // alias
...
// ── GET /evolution (Pro) — 12 mois glissants ─────────────────────
router.get('/evolution', async (req, res) => {
  ...
  try {
    ...
    res.json({
      months: result.map((r, i) => ({
        ...r,
        year:  months[i].y,
        label: MONTHS_FR[months[i].m - 1].slice(0, 3),
      }))
    })
  } catch (e) { res.status(500).json({ error: e.message }) }
})

// ── GET /annual (Pro) — bilan annuel ─────────────────────────────
router.get('/annual', async (req, res) => {
  const { year } = req.query
  try {
    const months = await Promise.all(
      Array.from({ length: 12 }, (_, i) => i + 1)
        .map(m => getMonthTotals(req.user.id, m, year))
    )
...
router.get('/export/pdf', async (req, res) => {
  ...
  try {
    ...
    const cards = [
      ...
      { x:L+(cardW+gap)*2,   title:'SOLDE NET', main:balance,      sub1:`Taux epargne : ${savingPct} %`,    sub2:`Prevision : ${fmtAr(balance)}`   },
    ]
    cards.forEach(c => {
      ...
      T(`${sign}${fmtAr(Math.abs(c.main))}`, c.x+8, cardY+23, { bold:true, size:13, w:cardW-16 })
    ...
    doc.y += 4; T(`Ponctuel : ${fmtAr(punctualInc)}   Recurent : ${fmtAr(recurInc)}`, L+10, doc.y, { size:8, color:'#555', w:PW }); doc.y += 18
    ...
    doc.y += 4; T(`Ponctuel : ${fmtAr(punctualExp)}   Recurent : ${fmtAr(recurExp)}`, L+10, doc.y, { size:8, color:'#555', w:PW }); doc.y += 18
    hLine(doc.y, 1); doc.y += 10

    newPageIfNeeded()
    T('SOLDE NET', L, doc.y, { bold:true, size:11, w:PW-120 })
    T(`${balance>=0?'+':''}${fmtAr(balance)}`, L, doc.y, { bold:true, size:11, w:PW, align:'right' })
    ...
    for (let i=0; i<pageCount; i++) {
      ...
      doc.fillColor('#888').font('Helvetica').fontSize(7)
         .text(`Expense Tracker  -  ${MONTHS_FR[Number(month)-1]} ${year}  -  Page ${i+1}/${pageCount}`, L, 830, { width:PW, align:'center', lineBreak:false })
    }
    doc.end()
  } catch (e) { res.status(500).json({ error: e.message }) }
})

// ── GET /export/annual/pdf (Pro) ──────────────────────────────────
router.get('/export/annual/pdf', async (req, res) => {
  ...
  try {
    ...
    doc.y = 80; hLine(doc.y, 1); doc.y += 16

    const cardW=150, cardH=80, cardY=doc.y, gap=(PW-cardW*3)/2
    ;[
      { title:'REVENUS ANNUELS',  main:totalInc },
      { title:'DÉPENSES ANNUELS', main:totalExp },
      { title:'ÉPARGNE NETTE',    main:balance, extra:`Taux : ${saving} %` },
    ].forEach((c, i) => {
      ...
      T(fmtAr(Math.abs(c.main)), x+8, cardY+22, { bold:true, size:12, w:cardW-16 })
      if (c.extra) T(c.extra, x+8, cardY+55, { size:8, color:'#555', w:cardW-16 })
    })
    doc.y = cardY + cardH + 20; hLine(doc.y, 1); doc.y += 14

    T('Détail mensuel', L, doc.y, { bold:true, size:12, w:PW }); doc.y += 14
    hLine(doc.y, 0.5); doc.y += 8

    const colW = [70, 120, 120, 90, 95]
    ;['Mois','Revenus','Dépenses','Solde','Taux épargne'].forEach((h, i) => {
    ...
    months.forEach((m, idx) => {
      ...
      const row = [MONTHS_FR[m.month-1], fmtAr(m.totalIncomes), fmtAr(m.totalExpenses), `${m.balance>=0?'+':''}${fmtAr(m.balance)}`, `${sr} %`]
      row.forEach((v, i) => {
    ...
    T(fmtAr(totalInc), L+colW[0],                    doc.y, { bold:true, size:10, w:colW[1], align:'right' })
    T(fmtAr(totalExp), L+colW[0]+colW[1],             doc.y, { bold:true, size:10, w:colW[2], align:'right' })
    T(`${balance>=0?'+':''}${fmtAr(balance)}`,         L+colW[0]+colW[1]+colW[2],        doc.y, { bold:true, size:10, w:colW[3], align:'right', color:balance>=0?'#00b894':'#e74c3c' })
    ...
    for (let i=0; i<pageCount; i++) {
      doc.switchToPage(i); hLine(822, 0.4, '#aaa')
      doc.fillColor('#888').font('Helvetica').fontSize(7)
         .text(`Expense Tracker  -  Bilan Annuel ${year}  -  Page ${i+1}/${pageCount}`, L, 830, { width:PW, align:'center', lineBreak:false })
    }
    doc.end()
  } catch (e) { res.status(500).json({ error: e.message }) }
})
...
router.get('/export/excel', async (req, res) => {
  ...
  try {
    ...
    const punctualExp = expenses.reduce((s,e) => s + Number(e.amount), 0) - generatedRecurExp
    const punctualInc = incomes.reduce((s,i)  => s + Number(i.amount), 0) - generatedRecurInc
    const totalExp    = punctualExp + recurExp
    const totalInc    = punctualInc + recurInc
    ...
    const sRow = (label, value, bold=false, bg=null) => {
      ...
      r.getCell(2).value=value; r.getCell(2).numFmt='#,##0 "Ar"'
    ...
    sRow('  Ponctuels', punctualInc); sRow('  Récurrents', recurInc); sRow('TOTAL REVENUS', totalInc, true, 'FFE8F5E9')
    wsS.addRow([])
    wsS.addRow({ l:'DÉPENSES', v:'' }); wsS.lastRow.getCell(1).font={bold:true,size:11}; wsS.lastRow.height=20
    sRow('  Ponctuelles', punctualExp); sRow('  Récurrentes', recurExp); sRow('TOTAL DÉPENSES', totalExp, true, 'FFFCE4E4')
    ...
    const addSheet = (name, tabColor, rows, totalVal) => {
      ...
      rows.forEach((r,i) => {
        ws.addRow({ date:new Date(r.date).toLocaleDateString('fr-FR'), category:r.category?.name||'-', desc:r.description||'-', type:r.isRecurring?'Récurrent':'Ponctuel', amount:Number(r.amount) })
        ...
        dr.getCell(5).numFmt='#,##0 "Ar"'; dr.getCell(5).font={bold:true,size:10}; dr.getCell(5).alignment=rAlign
      })
      ...
      tr.getCell(5).numFmt='#,##0 "Ar"'; tr.getCell(5).font={bold:true,size:11}; tr.getCell(5).fill=hFill('FFF0F0F0'); tr.getCell(5).alignment=rAlign; tr.getCell(5).border=allBdr
    }
