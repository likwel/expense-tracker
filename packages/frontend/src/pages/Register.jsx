...
import {
  Eye, EyeOff, ArrowRight, Check, User, Users,
  Building2, Search, Plus, X, Zap,
} from 'lucide-react'
...
const USAGE_TYPES = [
  {
    code: 'personal', label: 'Personnel', sub: 'Usage individuel',
    Icon: User, color: '#534AB7', bg: '#EEEDFE',
    planLabel: 'Pro', planPrice: '5 000 Ar/mois', planId: 'pro',
  },
  {
    code: 'family',   label: 'Famille',   sub: 'Jusqu\'à 5 membres',
    Icon: Users, color: '#0F6E56', bg: '#E1F5EE',
    planLabel: 'Famille', planPrice: '15 000 Ar/mois', planId: 'family',
  },
  {
    code: 'business', label: 'Business',  sub: 'Membres illimités',
    Icon: Building2, color: '#D85A30', bg: '#FAECE7',
    planLabel: 'Business', planPrice: '40 000 Ar/mois', planId: 'business',
  },
]

const PLAN_FEATURES = {
  personal: ['Transactions illimitées', 'Budgets & récurrents', 'Rapports complets', 'Multi-devises'],
  family:   ['Tout le plan Pro', 'Jusqu\'à 5 membres', 'Invitations par lien', 'Finances partagées'],
  business: ['Tout le plan Famille', 'Membres illimités', 'Gestion des rôles', 'Rapports d\'équipe'],
}
  ...
  return (
    <div style={{ marginTop: -8, marginBottom: 16 }}>
      ...
      <div style={{ display: 'flex', gap: 12 }}>
        {[{ ok: hasLen, text: '8 car.' }, { ok: hasNum, text: 'Chiffre' }, { ok: hasUp, text: 'Majuscule' }]
          .map(({ ok, text }) => (
            ...
            <button key={c.code} type="button" onClick={() => setF(p => ({ ...p, currency: c.code }))}
              ...
              <div style={{ textAlign: 'left' }}>
                <div style={{ fontSize: 12, fontWeight: 600,
                  color: f.currency === c.code ? '#3C3489' : '#444' }}>{c.code}</div>
...
// ── Étape 2 : Type d'utilisation + plan associé ───────────────────

function StepUsage({ f, setF }) {
  const selected = USAGE_TYPES.find(u => u.code === f.usageType)
  return (
    <div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 16 }}>
        {USAGE_TYPES.map(({ code, label, sub, Icon, color, bg, planLabel, planPrice }) => {
          const active = f.usageType === code
          return (
            <button key={code} type="button"
              onClick={() => setF(p => ({ ...p, usageType: code }))}
              style={{
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 14, fontWeight: 700,
                  color: active ? color : '#222' }}>{label}</div>
                <div style={{ fontSize: 11, color: '#aaa', marginTop: 1 }}>{sub}</div>
              </div>
              <div style={{ textAlign: 'right', flexShrink: 0 }}>
                <div style={{ fontSize: 11, fontWeight: 700,
                  color: active ? color : '#bbb' }}>{planLabel}</div>
                <div style={{ fontSize: 10, color: '#bbb' }}>{planPrice}</div>
              </div>
              {active && (
                <div style={{ width: 22, height: 22, borderRadius: 11, background: color,
      ...
      {selected && (
        <div style={{
          background: selected.bg, borderRadius: 12, padding: '12px 14px',
          border: `1px solid ${selected.color}22`,
        }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
            <Zap size={12} color={selected.color} fill={selected.color}/>
            <span style={{ fontSize: 11, fontWeight: 700, color: selected.color }}>
              2 mois d'essai gratuit · puis {selected.planPrice}
            </span>
          </div>
          {PLAN_FEATURES[selected.code].map(feat => (
            <div key={feat} style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 5 }}>
              <div style={{ width: 15, height: 15, borderRadius: 8, background: '#fff', flexShrink: 0,
                display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <Check size={9} color={selected.color} strokeWidth={3}/>
              </div>
              <span style={{ fontSize: 12, color: selected.color }}>{feat}</span>
            </div>
          ))}
        </div>
      )}
...
// ── Étape 3 : Organisation (family / business) ────────────────────
...
function StepOrg({ f, setF }) {
  ...
  const ut        = USAGE_TYPES.find(u => u.code === f.usageType)
  const color     = ut?.color ?? '#534AB7'
  const bg        = ut?.bg    ?? '#EEEDFE'
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      try {
        const { data } = await api.get(
          `/organizations/search?q=${encodeURIComponent(val)}&type=${f.usageType}`
        )
  ...
  const selectOrg = org => {
    ...
    setMode('join'); setQuery(org.name); setResults([])
  }
  const toggleCreate = () => {
  ...
  return (
    <div style={{ marginBottom: 8 }}>
      ...
      {/* Capacité selon plan */}
      <div style={{
        background: bg, borderRadius: 10, padding: '9px 12px', marginBottom: 14,
        border: `1px solid ${color}22`,
        display: 'flex', alignItems: 'center', gap: 6,
      }}>
        <TypeIcon size={13} color={color} strokeWidth={2}/>
        <span style={{ fontSize: 12, color, fontWeight: 600 }}>
          {f.usageType === 'family'
            ? 'Plan Famille · max 5 membres · 15 000 Ar/mois'
            : 'Plan Business · membres illimités · 40 000 Ar/mois'}
        </span>
      </div>
      ...
      {mode !== 'create' && (
        <div style={{ marginBottom: 12 }}>
          <div style={{
            ...
            />
            {loading && (
              <span style={{ width: 14, height: 14, border: '2px solid #ddd',
            ...
            {query.length > 0 && !loading && (
              <button type="button" onClick={clear}
                style={{ background: 'none', border: 'none', cursor: 'pointer',
                  color: '#ccc', padding: 0, flexShrink: 0 }}>
                <X size={13}/>
              </button>
            )}
          </div>
          ...
          {results.length > 0 && (
            <div style={{ marginTop: 8, display: 'flex', flexDirection: 'column', gap: 6 }}>
              {results.map(org => (
                <button key={org.id} type="button" onClick={() => selectOrg(org)} style={{
                  display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px',
                  borderRadius: 12, cursor: 'pointer', textAlign: 'left',
                  border: `1.5px solid ${f.orgId === org.id ? color : '#eee'}`,
                  background: f.orgId === org.id ? bg : '#fff',
                }}>
      ...
      {/* Créer */}
      {mode !== 'join' && (
        <button type="button" onClick={toggleCreate} style={{
        ...
        <div style={{ marginTop: 10 }}>
          ...
          <input
            ...
            style={{ width: '100%', boxSizing: 'border-box', padding: '11px 14px',
              ...
              fontSize: 14, color: '#222', outline: 'none', background: '#fafafa' }}
          />
        </div>
      )}
...
const STEP_LABELS = ['Compte', 'Type', 'Organisation']
...
export default function Register() {
  ...
  // Pré-sélection depuis la landing (?plan=family&months=1)
  const planParam = searchParams.get('plan')
  const initUsageType = (() => {
    if (planParam === 'family')   return 'family'
  ...
  // Remettre à jour usageType si le param change (navigation)
  useEffect(() => {
    if (planParam) setF(p => ({ ...p, usageType: initUsageType }))
  }, [planParam])

  const needsOrg   = f.usageType !== 'personal'
  const totalSteps = needsOrg ? 3 : 2
  const stepLabels = needsOrg ? STEP_LABELS : STEP_LABELS.slice(0, 2)
  ...
  const selectedUsage = USAGE_TYPES.find(u => u.code === f.usageType)
      ...
      <div style={{ width: '100%', maxWidth: 420, background: '#fff', borderRadius: 20,
        ...
        <h1 style={{ fontSize: 20, fontWeight: 800, color: '#222', marginBottom: 4, textAlign: 'center' }}>
          {step === 0 ? 'Créer un compte'
           : step === 1 ? 'Type d\'utilisation'
           : 'Votre organisation'}
        </h1>
        <p style={{ color: '#aaa', fontSize: 13, textAlign: 'center', marginBottom: 20 }}>
          {step === 0 ? 'Commencez à suivre vos finances'
           : step === 1 ? 'Choisissez votre plan · 2 mois offerts'
           : 'Rejoignez ou créez votre groupe'}
        </p>

        {/* Badge plan pré-sélectionné (si venu depuis landing) */}
        {step === 0 && planParam && selectedUsage && (
          <div style={{
            display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6,
            background: selectedUsage.bg, borderRadius: 10, padding: '9px 14px', marginBottom: 18,
            border: `1px solid ${selectedUsage.color}22`,
          }}>
            <selectedUsage.Icon size={13} color={selectedUsage.color} strokeWidth={2}/>
            <span style={{ fontSize: 12, color: selectedUsage.color, fontWeight: 600 }}>
              Plan {selectedUsage.planLabel} sélectionné · {selectedUsage.planPrice}
            </span>
          </div>
        )}

        {/* Badge essai par défaut (pas de pré-sélection) */}
        {step === 0 && !planParam && (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6,
            ...
            <span style={{ fontSize: 12, color: '#0F6E56', fontWeight: 600 }}>
              2 mois Pro offerts — sans carte bancaire
            </span>
          </div>
        )}
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 8 }}>
            ...
            <button
              ...
              style={{
                ...
                color: '#EEEDFE', fontWeight: 700, fontSize: 14,
