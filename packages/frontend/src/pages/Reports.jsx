...
import { Download, RefreshCw, Lock, TrendingUp, Calendar, BarChart2, ArrowRight, Zap } from 'lucide-react'
  ...
  return (
    <div onClick={onClose} style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        background: '#fff', borderRadius: '20px 20px 0 0', padding: '0 0 32px',
      }}>
        ...
        <div style={{ padding: '12px 20px 20px' }}>
          ...
          <div style={{ fontSize: 13, color: '#888', lineHeight: 1.6, marginBottom: 20 }}>
            <strong style={{ color: '#222' }}>{LABELS[feature]}</strong> est disponible avec le plan Pro.
          </div>
          {[
          ...
          <button onClick={() => { onClose(); navigate('/settings/plan') }} style={{
...
export default function Reports() {
  ...
  // ✅ isPro = plan payant actif (hors essai) OU essai actif
  // Les features Pro sont disponibles en essai aussi
  const hasProFeatures = !isFree   // trial + paid = accès Pro features
  // ✅ Signature dans PDF uniquement si plan gratuit ou essai (pas plan payant confirmé)
  const pdfShowSignature = isFree || isTrial  // false si isPaid

  const { activeOrg } = useOrg()
  // ✅ Paramètres org injectés dans tous les appels API et exports
  const orgParam = activeOrg ? { orgId: activeOrg.id } : {}
  const PARAMS   = { month, year, ...orgParam }
  ...
  const { data: evolution }    = useApi(hasProFeatures ? '/reports/evolution' : null, { year, ...orgParam })
  const prevMonth              = month === 1 ? 12 : month - 1
  const prevYear               = month === 1 ? year - 1 : year
  const { data: prevSummary }  = useApi(hasProFeatures ? '/reports/summary' : null, { month: prevMonth, year: prevYear, ...orgParam })
  const { data: annualData }   = useApi(hasProFeatures ? '/reports/annual'  : null, { year, ...orgParam })
  ...
  const estimateAmount = (list) => list.reduce((s, r) => {
  ...
  const estimatedExp = estimateAmount(activeRecurExp)
  const estimatedInc = estimateAmount(activeRecurInc)
  ...
  /* ── Export — injecte orgId si org active + pdfShowSignature ── */
  const exportPDF = async () => {
    try {
      const params = { month, year, ...orgParam }
      // pdfShowSignature est lu côté backend via le plan de l'user
      // (le backend décide lui-même selon isTrial/plan)
      const r = await reportsApi.pdf(params)
      const label = activeOrg ? `org${activeOrg.id}-` : ''
      download(r.data, `rapport-${label}${year}-${String(month).padStart(2, '0')}.pdf`)
    } catch { alert('Erreur export PDF') }
  }

  const exportExcel = async () => {
    try {
      const r = await reportsApi.excel({ month, year, ...orgParam })
      const label = activeOrg ? `org${activeOrg.id}-` : ''
      download(r.data, `rapport-${label}${year}-${String(month).padStart(2, '0')}.xlsx`)
    } catch { alert('Erreur export Excel') }
  }

  const exportAnnual = async () => {
    try {
      const r = await reportsApi.annualPdf({ year, ...orgParam })
      const label = activeOrg ? `org${activeOrg.id}-` : ''
      download(r.data, `rapport-annuel-${label}${year}.pdf`)
    } catch (e) { console.error(e); alert('Erreur export rapport annuel') }
  }
      ...
      <div style={{ padding: '12px 16px' }}>
        ...
        {activeOrg && (
          <div style={{
            ...
            background: '#EEEDFE', borderRadius: 10, padding: '9px 14px',
            border: '1px solid #AFA9EC22',
          }}>
            <div style={{
              width: 20, height: 20, borderRadius: 6, background: '#534AB7',
              display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0,
            }}>
              <BarChart2 size={11} color="#fff" strokeWidth={2}/>
            </div>
            <span style={{ fontSize: 12, color: '#3C3489', fontWeight: 600 }}>
              Rapport pour : <strong>{activeOrg.name}</strong>
            </span>
          </div>
        )}
        ...
        {/* Hero taux d'épargne */}
        <div style={{
          background: '#00b894', borderRadius: 20,
          ...
          <div style={{ display: 'flex', marginTop: 14 }}>
            {[
              { label: `Revenus${isIncEstimated ? ' ~' : ''}`, val: totalInc },
              { label: `Dépenses${isExpEstimated ? ' ~' : ''}`, val: totalExp },
              { label: 'Solde', val: balance, color: balance >= 0 ? '#fff' : '#ffb3b3' },
            ].map(({ label, val, color }, i) => (
              <>
                {i > 0 && <div style={{ width: 1, background: 'rgba(255,255,255,0.25)', margin: '0 10px' }}/>}
                <div key={label} style={{ flex: 1 }}>
                  <div style={{ fontSize: 10, opacity: 0.7, marginBottom: 2 }}>{label}</div>
                  <div style={{ fontWeight: 700, fontSize: 13, color: color || '#fff' }}>{fmt(val)}</div>
                </div>
              </>
            ))}
          </div>
        ...
        {hasProFeatures ? (
          <Card>
            <div style={{ fontWeight: 700, fontSize: 15, color: '#222', marginBottom: 4 }}>Évolution des dépenses</div>
            <div style={{ fontSize: 11, color: '#aaa', marginBottom: 14 }}>6 derniers mois</div>
            <BarChart data={evoData} color="#6C5CE7"/>
          </Card>
        ) : (
          <ProCard feature="evolution" title="Évolution sur 6 mois" icon={BarChart2}
        ...
        {hasProFeatures ? (
          <Card>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              {[
                ...
                <div key={label} style={{ background: '#f9f9f9', borderRadius: 10, padding: '12px', border: '1px solid #f0f0f0' }}>
                  ...
                  <div style={{ fontSize: 11, fontWeight: 700, color: diff === 0 ? '#aaa' : (neg ? diff > 0 : diff < 0) ? '#e74c3c' : '#00b894' }}>
          ...
          <ProCard feature="comparison" title="Comparaison mois précédent" icon={ArrowRight}
            onUpgrade={setProModal}
            preview={
              <div>
        ...
        {(() => {
          ...
          return hasProFeatures ? (
            ...
            <ProCard feature="forecast" title="Prévisions mois suivant" icon={TrendingUp}
              onUpgrade={setProModal}
              preview={
                <div>
        ...
        {hasProFeatures ? (
          ...
          <ProCard feature="annual" title="Rapport annuel complet" icon={Calendar}
            onUpgrade={setProModal}
            preview={
              <div>
        ...
        <Card>
          <div style={{ fontWeight: 700, fontSize: 15, color: '#222', marginBottom: 4 }}>
            ...
            {activeOrg && (
              <span style={{ fontSize: 11, color: '#7F77DD', fontWeight: 600, marginLeft: 8 }}>
                · {activeOrg.name}
              </span>
            )}
          </div>
          {/* Mention signature pour trial/gratuit */}
          {pdfShowSignature && (
            <div style={{
              fontSize: 11, color: '#888', background: '#EEEDFE',
              borderRadius: 8, padding: '6px 10px', marginBottom: 10, marginTop: 4,
            }}>
              Les PDF incluent une signature Depenzo (plan {isTrial ? 'Essai' : 'Gratuit'}).
              <span
                onClick={() => navigate('/settings/plan')}
                style={{ color: '#534AB7', fontWeight: 700, marginLeft: 4, cursor: 'pointer' }}>
                Passer au Pro →
              </span>
            </div>
          )}
